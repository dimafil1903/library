@if(isset($options->model) && isset($options->type))
  ...
  $selected_values = $model::where($options->column, '=', $relationshipData->{$options->key})->get()->map(function ($item, $key) use ($options) {
                    ...
                    if ($options->model==\App\Models\TeacherCard::class){


                    $selected_values = $model::where($options->column, '=', $relationshipData->{$options->key})->get()->map(function ($item, $key) use ($options) {
                        return [
                            "Книга"=>$item->book->title,
                            "Дата отримання"=> $item->took_at,
                            "Дата повернення"=> $item->return_at,
                            "Повернута"=> $item->returned ? "так":"ні",
                            "Бібліотекар"=>$item->library_staff->name
                            ];
                    })->all();
                    }
                ...
                @if($view == 'browse')
                    ...
                    @if(empty($selected_values))
                        <p>{{ __('voyager::generic.no_results') }}</p>
                    @elseif(is_string($selected_values))
                        <p>{{ $string_values }}</p>

                    @elseif(is_array($selected_values))
